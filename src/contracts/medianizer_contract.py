...
def Medianizer(
    ...
    sort_1 = Seq([
        ...
        .Then(
            Seq(a.slot.store(b), b.slot.store(tmp.load()))
            )
        .ElseIf(And(c > b, c > a, c > d, c > e))
        .Then(
            Seq(a.slot.store(c), c.slot.store(tmp.load()))
            )
        .ElseIf(And(d > b, d > a, d > c, d > e))
        .Then(
            Seq(a.slot.store(d), d.slot.store(tmp.load()))
            )
        .ElseIf(And(e > b, e > a, e > c, e > d))
        .Then(
            Seq(a.slot.store(e), e.slot.store(tmp.load()))
            )
        ])
    ...
    sort_2 = Seq([
        ...
        If(And(c > b, c > d, c > e))
        .Then(
            Seq(b.slot.store(c), c.slot.store(tmp.load()))
            )
        .ElseIf(And(d > b, d > c, d > e))
        .Then(
            Seq(b.slot.store(d), d.slot.store(tmp.load()))
            )
        .ElseIf(And(e > b, e > c, e > d))
        .Then(
            Seq(b.slot.store(e), e.slot.store(tmp.load()))
            )
    ])
    sort_3 = Seq([
        tmp.store(c),
        If(And(d > c, d > e))
        .Then(
            Seq(c.slot.store(d), d.slot.store(tmp.load()))
            )
        .ElseIf(And(e > c, e > d))
        .Then(
            Seq(c.slot.store(e), e.slot.store(tmp.load()))
            )
    ])
    ...
    sort_4 = Seq([
        ...
        If(e > d)
        .Then(
            Seq(d.slot.store(e), e.slot.store(tmp.load()))
            )
    ])
...
def get_values():
    ...
    validate_prices = Seq([
        If(And(feed_1_value.hasValue(), (median_timestamp - Global.latest_timestamp()) > time_interval))
        .Then(var1.store(feed_1_value.value()))
